...
LOG_MODULE_DECLARE(AD7156, CONFIG_SENSOR_LOG_LEVEL);
...
static void ad7156_gpio_callback(const struct device *dev,
				  struct gpio_callback *cb, uint32_t pins)
{
	struct ad7156_data *drv_data =
		CONTAINER_OF(cb, struct ad7156_data, gpio_cb);

	handle_int(drv_data->dev);
}

#if defined(CONFIG_AD7156_TRIGGER_OWN_THREAD)
static void ad7156_thread(struct ad7156_data *drv_data)
{
	while (true) {
...
#elif defined(CONFIG_AD7156_TRIGGER_GLOBAL_THREAD)
static void ad7156_work_cb(struct k_work *work)
{
	struct ad7156_data *drv_data =
		CONTAINER_OF(work, struct ad7156_data, work);

	process_int(drv_data->dev);
}
#endif

int ad7156_trigger_set(const struct device *dev,
...
int ad7156_init_interrupt(const struct device *dev)
{
	...
	k_thread_create(&drv_data->thread, drv_data->thread_stack,
			CONFIG_AD7156_THREAD_STACK_SIZE,
			(k_thread_entry_t)ad7156_thread, drv_data,
			NULL, NULL, K_PRIO_COOP(CONFIG_AD7156_THREAD_PRIORITY),
			0, K_NO_WAIT);
#elif defined(CONFIG_AD7156_TRIGGER_GLOBAL_THREAD)
	drv_data->work.handler = ad7156_work_cb;
#endif
